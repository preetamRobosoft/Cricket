...
class RegistrationViewModel {
    ...
    func verifyOTP(completion: @escaping((Bool) -> Void)) {
        ...
        print(verifyOTPParameters, verifyOTPHeaders)
        let networkManger = NetworkManager()
        networkManger.postData(url: verifyOTPURL, parameters: verifyOTPParameters, headers: verifyOTPHeaders, image: nil) { (statusCode, error) in
            if error != nil {
                completion(false)
